...
def get_LRIstream_runtime(js):
    ...
    def latest_rundate():
        '''
        This function returns the latest rundate of the batch based on the current day.
        '''
    ...
    #         print (cur_row)
            ...
            prev_row.update(cur_row)
            prev_row['ROW_NUM_OLD'] = cur_row['ROW_NUM']
            prev_row['ROW_NUM'] = val
            return val
        return wrapper
    ...
    #################################### Calculate ETA / Completion time ###################################

    ETA_STR = ""
    ETA_STR +=  f'''
             Now it is {pd.Timestamp.now('Europe/London').strftime('%H:%M:%S')} HRS (London Time)
          The Jobstream usually takes approximately {round(df_final['AVG_DURATION'].sum(),2)} mins. (May be 10 mins more or less.)'''
    ...
    if (df_final['DURATION_TODAY'].isnull().any()):
        ...
        ETA_STR += f'''          Today it has taken {round(so_far,2)} mins so far.
          It may complete by approximately {ETA} HRS London Time.'''
    else:
        ETA_STR +=f'''          Today it took {round(tot_time,2)} mins to complete.'''
...
def get_FDMstream_runtime(js):
    ...
    ETA_STR +=  f'''
          Now it is {pd.Timestamp.now('Europe/London').strftime('%H:%M:%S')} HRS (London Time)
          The Jobstream usually takes approximately {round(df_chart['AVG_DURATION'].sum(),2)} mins. (May be 10 mins more or less.)'''
    TN = pd.Timestamp.now('Europe/London') # What is the time now? and then convert it to London time zone
    ...
    if (df_chart['RUN_TIME'].isnull().any()):
        ...
        ETA = pd.to_datetime(df_chart[~df_chart['RUN_TIME'].isnull()]['RUN_TIME'].values[-1])\
         ...
         + T_FRM_LAST + T_REM
        ETA_STR += f'''          Today it has taken {round(so_far,2)}  mins so far(for the completed tasks).
          It may complete by {ETA} HRS GMT.'''
    else:
        ETA_STR +=  f'''          Today it took {round(df_chart['DURATION'].astype('float').sum(), 2)} mins to complete.'''
